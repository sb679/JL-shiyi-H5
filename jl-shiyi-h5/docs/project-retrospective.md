...
9. **不要以为本地有代码就是已部署**：必须确认 `git status` 无未提交文件、`git remote -v` 有地址、`git push` 成功、且推到了 CI 监听的正确分支。
10. **不要嵌套 Git 仓库**：子目录内有 `.git` 时，外层仓库会将其当作 submodule（指针），GitHub 上看起来有文件但实际只有引用哈希（mode 160000）。部署前检查：`git ls-files -s <子目录>` 如果显示 `160000` 而非 `100644`，说明是 submodule 而非真实文件。
11. **不要混淆 CI 触发分支**：`ci.yml` 中 `on.push.branches` 指定了哪些分支会触发 CI，在 feature 分支上 push 不会触发。必须推送到 `main`（或 CI 配置中指定的分支）。
12. **不要忘记 `.gitignore`**：首次 commit 前必须创建 `.gitignore`，排除 `node_modules`、`.env`、`dist`、日志文件等。否则会把这些不该提交的文件推上 GitHub。
13. **不要以为 webhook 部署会自动触发**：GitHub Actions 的 deploy job 依赖 `build` 通过、分支匹配 `refs/heads/main`、`secrets.ECS_HOST` 和 `secrets.DEPLOY_WEBHOOK_SECRET` 配置正确。
...
### 3.11 代码未推送导致 ECS 部署不生效（2026-06-11）

用户发现 ECS 上运行的版本缺少管理员面板、头像上传、个人资料编辑等新功能，而这些功能在本地代码中已经实现。

排查过程：

1. 确认本地 `src/App.tsx`、`src/types.ts`、`server/` 等文件确实包含新功能代码。
2. ECS 健康检查返回正常，但功能缺失，说明 ECS 运行的是旧代码。
3. 检查 Git 状态：`git status` 显示所有文件都是 `??`（untracked），**从未 commit**。
4. 检查 Git remote：`git remote -v` 为空，**从未配置 remote**。
5. 检查 Git 分支：`git log` 报 `does not have any commits yet`。

**根因 1：代码从未 commit/push**

```
本地代码 (feature/trade-status-display 分支)
    → 0 commits, 0 remotes
    → GitHub Actions 从未触发
    → ECS webhook 从未收到部署请求
    → ECS 运行旧版本
```

**根因 2：jl-shiyi-h5 被当作 Git submodule**

`jl-shiyi-h5` 目录内有独立的 `.git`，外层仓库 `git add` 时将其识别为 submodule（mode 160000），只存储提交哈希引用，而非真实文件内容。GitHub 上看到的 `jl-shiyi-h5` 只是一个指针，**不包含任何源代码**。

修复步骤：

```powershell
# 1. 创建 .gitignore（过滤 node_modules、.env、日志等）
# 2. 添加 GitHub remote
git remote add origin https://github.com/sb679/JL-shiyi-H5.git

# 3. 首次 commit
git add . && git commit -m "feat: 首次提交发布"

# 4. 推送到 main 分支（CI 只监听 main）
git checkout -b main && git push -u origin main

# 5. 修复 submodule 嵌套问题
git rm --cached jl-shiyi-h5          # 移除 submodule 引用
rmdir /s /q jl-shiyi-h5\.git          # 删除嵌套 .git
git add jl-shiyi-h5/                  # 重新添加为真实文件
...
CI 配置 (`ci.yml`) 中 `on.push.branches: [main]`，只监听 `main` 分支的 push。本地工作在 `feature/trade-status-display` 分支，即使 push 了也不会触发 CI。
...
push to main → GitHub Actions 触发
  ├── test job: lint + 单元测试
  ├── build job: 构建生产包 (依赖 test 通过)
  └── deploy job: curl 调用 ECS webhook → 部署 (依赖 build 通过)
```

经验：

- **不要假设代码 push 了就是部署了**：必须确认 Git 状态（有 commit）、有 remote、push 到了正确的分支。
- **嵌套 Git 仓库是常见陷阱**：子目录有 `.git` 时，外层仓库会将其视为 submodule（指针），而非真实文件。GitHub 上看起来文件存在，实际只是 160000 模式引用。
- **CI 只监听配置中指定的分支**：在 feature 分支上工作不会触发 CI，需要合并到 main 或修改 CI 配置。
- **`.gitignore` 必须在首次 commit 前创建**：避免 `node_modules`、`.env`、日志文件等被意外提交。
- **ECS 自动更新脚本**（`windows-ecs-enable-auto-update-gitee.ps1`）每 5 分钟从 Gitee 拉取代码，但前提是代码已推送到 Gitee。
- **GitHub Actions webhook 部署**依赖 CI 通过后触发，但前提是代码已推送到 GitHub 且 CI 配置正确。
