...
如果希望使用 Windows 系统环境变量而不是 `C:\jl-shiyi-h5\.env`，可以在 ECS PowerShell 中运行：

```powershell
powershell -ExecutionPolicy Bypass -File C:\jl-shiyi-h5\scripts\windows-ecs-config-env.ps1
```
...
不要从 `C:\wwwroot\JL-shiyi-H5\scripts\windows-ecs-deploy.ps1` 跑更新：`C:\wwwroot\JL-shiyi-H5` 是静态镜像目录，不是 Git 主目录。首次部署或需要重新填写 OSS/RDS 配置时，使用 `C:\jl-shiyi-h5\scripts\windows-ecs-deploy.ps1`；日常拉新代码用上面的 `windows-ecs-update.ps1`。
