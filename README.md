...
不要从 `C:\wwwroot\JL-shiyi-H5\scripts\windows-ecs-deploy.ps1` 跑更新：`C:\wwwroot\JL-shiyi-H5` 是静态镜像目录，不是 Git 主目录。首次部署或需要重新填写 OSS/RDS 配置时，使用 `C:\jl-shiyi-h5-gitee\scripts\windows-ecs-deploy-gitee.ps1`；日常拉新代码用上面的 `windows-ecs-update-gitee.ps1`。

更新脚本会保留服务器本地 `.env`，检查 Gitee 是否有新 commit。没有新代码会直接退出；有新代码才会拉取、构建并重启服务。只有依赖文件变化时才会重新 `npm ci`。
...
```powershell
node C:\jl-shiyi-h5-gitee\server\deploy-webhook.js
```

**或者**注册为 Windows 计划任务实现开机自启（推荐）：
```powershell
$Action = New-ScheduledTaskAction -Execute "node" -Argument "C:\jl-shiyi-h5-gitee\server\deploy-webhook.js"
$Trigger = New-ScheduledTaskTrigger -AtStartup
$Principal = New-ScheduledTaskPrincipal -UserId "SYSTEM" -RunLevel Highest
Register-ScheduledTask -TaskName "JL拾遗 Webhook" -Action $Action -Trigger $Trigger -Principal $Principal -Force
Start-ScheduledTask -TaskName "JL拾遗 Webhook"
```

**不要使用旧版 `windows-ecs-enable-auto-update.ps1`**（指向已废弃的 `C:\jl-shiyi-h5` 目录且使用 GitHub 拉取）。

Webhook 监听 `DEPLOY_WEBHOOK_PORT`（默认 9000）接收 GitHub Actions 的 POST 请求。该端口需要在阿里云安全组中放行入方向 TCP 9000。
...
**重要：webhook 从 GitHub 拉取代码，而不是 Gitee。** 如果 ECS 无法访问 GitHub（国内常见），webhook 的 `git pull` 会失败。此时需改用 Gitee 5 分钟轮询方案（`windows-ecs-enable-auto-update-gitee.ps1`）。
...
ECS 上的 webhook 服务（`server/deploy-webhook.js`）收到请求后依次执行 `git pull origin main → npm ci → npm run build → wwwroot 同步 → 重启 Windows Scheduled Task`。
