...
- `C:\jl-shiyi-h5` 是主项目目录，安装依赖、构建、启动 Node、保存 `.env` 都应以它为准。
...
scripts/windows-ecs-update.ps1          # 日常更新脚本，拉 GitHub、构建、同步 dist、重启服务
scripts/windows-ecs-enable-auto-update.ps1 # 注册每 5 分钟自动更新任务
...
```powershell
# 首次部署（在 ECS 上）
powershell -ExecutionPolicy Bypass -File "C:\jl-shiyi-h5\scripts\windows-ecs-deploy-gitee.ps1"

# 日常更新（在 ECS 上）
powershell -ExecutionPolicy Bypass -File "C:\jl-shiyi-h5\scripts\windows-ecs-update-gitee.ps1"

# 启用自动更新（每 5 分钟从 Gitee 拉取）
powershell -ExecutionPolicy Bypass -File "C:\jl-shiyi-h5\scripts\windows-ecs-enable-auto-update-gitee.ps1"
```
...
4. 不要在 `PS C:\jl-shiyi-h5>` 里删除 `C:\jl-shiyi-h5` 自己。
